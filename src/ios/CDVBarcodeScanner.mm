...
NSMutableDictionary *qrCodeArray = [[NSMutableDictionary alloc] init];
NSUInteger total = 0;
...
- (void)captureOutput:(AVCaptureOutput*)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection*)connection {
    ...
    try {
        ...
        AVMetadataObject *qrMetaData;
        // This will bring in multiple entities if there are multiple 2D codes in frame.
        for (AVMetadataObject *metaData in metadataObjects) {
            
            qrMetaData = metaData;
        
            // Get the hexa buyte code from AV Foundation
            NSString *hexData = [[[qrMetaData valueForKey:@"_internal"] valueForKey:@"basicDescriptor"] valueForKey:@"BarcodeRawData"];
                
            NSMutableArray *hexDataArray = [self ceateDecimalArrayWithHexString:hexData];
            
            AVMetadataMachineReadableCodeObject* code = (AVMetadataMachineReadableCodeObject*)[self.previewLayer transformedMetadataObjectForMetadataObject:(AVMetadataMachineReadableCodeObject*)qrMetaData];
        
 
            // Checking if this one is Structured QR Code array, otherwise it send the result directly.
            if([hexDataArray[0] isEqual:@"3"]){
                NSUInteger lastTotal = [self convertHextDecimal:hexDataArray[2]];
                
                // For the first time scan
                if(total == 0){
                    total = lastTotal;
                }
                ...
                if(error == false && qrCodeArray.count == (total + 1)){
                    NSString *resultCode = @"";
                    ...
                    // Final return of Total code
                    [self barcodeScanSucceeded:resultCode format:[self formatStringFromMetadata:code]];
                }
                
                
            }
            // Single QR Code found
            else{
                completed = true;
                if ([self checkResult:code.stringValue] && completed == true) {
                    AudioServicesPlaySystemSound(_soundFileObject);
...
-(NSMutableArray *) ceateDecimalArrayWithHexString:(NSString *)theData
{
    ...
    for (int i = 0; i < [str length]; i++) {
        [charArray addObject:[NSString stringWithFormat:@"%C", [str characterAtIndex:i]]];
    }
...
-(NSUInteger)convertHextDecimal:(NSString *) str
{
    unsigned result = 0;
...
- (id)initWithProcessor:(CDVbcsProcessor*)processor alternateOverlay:(NSString *)alternateXib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(updateScanStatus:)
                                                 name:@"QRCodeFound"
                                               object:nil];
